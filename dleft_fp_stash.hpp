...
class DleftFpStash {
 public:
  DleftFpStash(size_t = 0);
	...
	// Returns `true` if insertion/update is successful and `false` otherwise (e.g. when running out of space)
...
auto DLEFT_TYPE::Bucket::Append(const KeyType &key, const ValueType &value, uint32_t hash, StashBucket *stash_bucket) -> bool {
	...
	if (stash_bucket->InsertMajorOverflow(key, value, hash)) {  // Minor overflow slots used up; Insert as a major overflow
		overflow_count_++;
		return true;
	}
	return false;
...
auto DLEFT_TYPE::StashBucket::InsertMajorOverflow(const KeyType &key, const ValueType &value, uint32_t hash) -> bool {
	...
	return AppendMajorOverflow(key, value, hash);  // Insert at an empty slot
}
